...
# import devanagari file
import matplotlib.pyplot as plt
import easyocr
from pylab import rcParams
from IPython.display import Image

img = cv2.imread('dataset\dev\hindi1.jpg')
######################################################################################################
...
flag= int(input("is devnagari?(press 1 for yes else 0): "))
if(flag==0):
...
#cv2.imshow('sample image', img)
   cv2.waitKey(0)
   cv2.destroyAllWindows()
   #text = reader.readtext(gray_plates)
   text = reader.readtext(img)
   print("licence plate",text[1][1])
